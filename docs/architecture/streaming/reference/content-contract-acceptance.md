# CONTENT-CONTRACT Content Contract Enforcement Acceptance Reference
...
Out of scope:
1. CDC contract enforcement for `cdc.content.videos` (`CDC-CONTRACT`).

Runtime defaults (CONTENT-CONTRACT):
...
Current repo note:
1. There is no dedicated `run_content_contract_acceptance.sh` wrapper in this repo.
2. Use the manual verifier command below when you need content-contract-only validation.
