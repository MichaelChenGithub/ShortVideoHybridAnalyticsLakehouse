# CDC-UPSERT CDC Upsert Acceptance Reference
...
Out of scope:
1. CDC quarantine/contract enforcement (`CDC-CONTRACT`).
2. `content_events` pipeline validation (`CONTENT-CONTRACT`/`CONTENT-AGGREGATOR`).

Runtime defaults (CDC-UPSERT):
