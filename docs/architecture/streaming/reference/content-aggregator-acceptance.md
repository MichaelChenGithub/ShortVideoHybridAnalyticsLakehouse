# CONTENT-AGGREGATOR Content Aggregator Bring-up Acceptance Reference
...
Out of scope:
1. `invalid_events_content` contract enforcement validation (`CONTENT-CONTRACT`).
2. CDC pipeline validation (`CDC-UPSERT`/`CDC-CONTRACT`).

Runtime defaults (CONTENT-AGGREGATOR):
