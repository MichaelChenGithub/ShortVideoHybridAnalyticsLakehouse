# Kafka Topic, Schema, and Retention Contract
...
5. automated notification/alert routing workflows
...
### 3.3 Consumer Naming and Consumer Group IDs (Baseline)
...
### 4.2 Baseline Configuration (local/dev)
...
Allowed `event_type` values (baseline):
...
1. additive-first changes only in baseline scope
...
4. incompatible messages are quarantined to source-specific invalid tables in current scope

Compatibility target:

1. consumers must tolerate unknown fields in `payload_json`
2. invalid records in current scope are quarantined to data-table workflow, not Kafka DLQ topics
3. quarantine routing in current scope:
...
## 9. Monitoring and Alerts (Baseline)
