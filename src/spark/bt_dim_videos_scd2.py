"""Batch transform for lakehouse.dims.dim_videos_scd2."""
...
try:
    from spark.bt_dim_videos_scd2_sql import (
    ...
    from bt_dim_videos_scd2_sql import (
        DIM_VIDEOS_SCD2_TABLE,
        OPEN_END_VALID_TO,
        create_dim_videos_scd2_sql,
        overwrite_dim_videos_scd2_from_raw_sql,
...
RAW_CDC_TABLE = "lakehouse.bronze.raw_cdc_videos"

ENV_SOURCE_TABLE = "BT_DIM_VIDEOS_SCD2_SOURCE_TABLE"
ENV_TARGET_TABLE = "BT_DIM_VIDEOS_SCD2_TARGET_TABLE"
ENV_APP_NAME = "BT_DIM_VIDEOS_SCD2_APP_NAME"
...
def load_job_settings(env: Mapping[str, str] | None = None) -> JobSettings:
    ...
    return JobSettings(
        app_name=values.get(ENV_APP_NAME, "spark_bt_dim_videos_scd2"),
        source_table=values.get(ENV_SOURCE_TABLE, RAW_CDC_TABLE),
        target_table=values.get(ENV_TARGET_TABLE, DIM_VIDEOS_SCD2_TABLE),
    )
...
def run_batch(settings: JobSettings) -> None:
    ...
    spark.sql(create_dim_videos_scd2_sql(settings.target_table))
