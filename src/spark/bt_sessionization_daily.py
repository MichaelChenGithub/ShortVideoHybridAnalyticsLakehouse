"""Batch transform for lakehouse.gold.batch_sessionization_daily."""
...
try:
    from spark.bt_sessionization_daily_sql import (
    ...
    from bt_sessionization_daily_sql import (
        BATCH_SESSIONIZATION_DAILY_TABLE,
        USER_ACTIVITY_SESSIONS_30M_TABLE,
        create_batch_sessionization_daily_sql,
        default_d_minus_1_data_date_sql,
...
ENV_SOURCE_TABLE = "BT_SESSIONIZATION_DAILY_SOURCE_TABLE"
ENV_TARGET_TABLE = "BT_SESSIONIZATION_DAILY_TARGET_TABLE"
ENV_APP_NAME = "BT_SESSIONIZATION_DAILY_APP_NAME"
ENV_DATA_DATE = "BT_SESSIONIZATION_DAILY_DATA_DATE"
...
def load_job_settings(env: Mapping[str, str] | None = None) -> JobSettings:
    ...
    return JobSettings(
        app_name=values.get(ENV_APP_NAME, "spark_bt_sessionization_daily"),
        source_table=values.get(ENV_SOURCE_TABLE, USER_ACTIVITY_SESSIONS_30M_TABLE),
        target_table=values.get(ENV_TARGET_TABLE, BATCH_SESSIONIZATION_DAILY_TABLE),
...
def run_batch(settings: JobSettings) -> None:
    ...
    spark.sql(create_batch_sessionization_daily_sql(settings.target_table))
    spark.sql(
        delete_batch_sessionization_daily_data_date_sql(
