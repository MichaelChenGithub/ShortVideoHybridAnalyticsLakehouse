"""Spark streaming job for user CDC raw bronze landing and invalid quarantine."""
...
try:
    from spark.rt_user_cdc_raw_contract import JobSettings, load_job_settings
    ...
    from rt_user_cdc_raw_contract import JobSettings, load_job_settings
    from rt_user_cdc_raw_sql import (
        create_invalid_events_cdc_users_sql,
        create_raw_cdc_users_sql,
        manual_alter_invalid_events_cdc_users_statements,
        manual_alter_raw_cdc_users_statements,
...
def init_output_tables(spark: SparkSession, settings: JobSettings) -> None:
    ...
    spark.sql(create_raw_cdc_users_sql(settings.raw_cdc_users_table))
    spark.sql(create_invalid_events_cdc_users_sql(settings.invalid_cdc_users_table))

    raw_columns = list(_table_columns(spark, settings.raw_cdc_users_table))
    raw_missing = missing_raw_cdc_users_columns(raw_columns)
    if raw_missing:
        _raise_missing_columns_error(
            settings.raw_cdc_users_table,
            raw_missing,
            manual_alter_raw_cdc_users_statements(raw_columns, settings.raw_cdc_users_table),
        )

    invalid_columns = list(_table_columns(spark, settings.invalid_cdc_users_table))
    invalid_missing = missing_invalid_events_cdc_users_columns(invalid_columns)
    if invalid_missing:
        _raise_missing_columns_error(
            settings.invalid_cdc_users_table,
            invalid_missing,
            manual_alter_invalid_events_cdc_users_statements(
                invalid_columns,
                settings.invalid_cdc_users_table,
            ...
            StructField(
                ...
                StructType(
                    [
                        ...
                        StructField("new_vs_returning_user", StringType(), True),
...
def read_user_cdc_stream(spark: SparkSession, settings: JobSettings) -> DataFrame:
    raw_stream = (
    ...
    return (
        ...
        .select(
            ...
            col("cdc.after.user_id").alias("user_id"),
            col("cdc.after.new_vs_returning_user").alias("new_vs_returning_user"),
...
def split_valid_and_invalid_rows(df: DataFrame) -> tuple[DataFrame, DataFrame]:
    valid_condition = (
        ...
        & col("new_vs_returning_user").isNotNull()
    ...
    error_code = (
        ...
        .when(col("user_id").isNull(), lit("CDC_MISSING_AFTER_USER_ID"))
        .when(col("new_vs_returning_user").isNull(), lit("CDC_MISSING_AFTER_USER_STATE"))
        .when(col("region").isNull(), lit("CDC_MISSING_AFTER_REGION"))
    ...
    error_reason = (
        ...
        .when(
            ~col("op").isin("c", "u"),
            lit("op must be one of c/u for user CDC ingestion scope"),
        )
        ...
        .when(col("user_id").isNull(), lit("required field after.user_id is missing"))
        .when(
            col("new_vs_returning_user").isNull(),
            lit("required field after.new_vs_returning_user is missing"),
        )
        .when(col("region").isNull(), lit("required field after.region is missing"))
...
def process_raw_batch(df: DataFrame, batch_id: int, table_name: str) -> None:
    count_rows = df.count()
    if count_rows == 0:
        print(f"Batch {batch_id}: no valid raw user CDC rows.")
        return

    print(f"Batch {batch_id}: writing {count_rows} raw user CDC rows to {table_name}.")
...
def process_invalid_batch(df: DataFrame, batch_id: int, table_name: str) -> None:
    count_rows = df.count()
    if count_rows == 0:
        print(f"Batch {batch_id}: no invalid user CDC records.")
        return

    print(f"Batch {batch_id}: writing {count_rows} invalid user CDC records to {table_name}.")
    df.write.format("iceberg").mode("append").save(table_name)
...
def main() -> None:
    ...
    cdc_stream = read_user_cdc_stream(spark, settings)
    raw_rows, invalid_rows = split_valid_and_invalid_rows(cdc_stream)

    raw_table_columns = list(_table_columns(spark, settings.raw_cdc_users_table))
    invalid_table_columns = list(_table_columns(spark, settings.invalid_cdc_users_table))
    ...
    query_raw = (
        raw_rows.writeStream.foreachBatch(
            lambda df, batch_id: process_raw_batch(df, batch_id, settings.raw_cdc_users_table)
        )
        .option("checkpointLocation", settings.checkpoint_raw_cdc_users)
    ...
    query_invalid = (
        invalid_rows.writeStream.foreachBatch(
            lambda df, batch_id: process_invalid_batch(df, batch_id, settings.invalid_cdc_users_table)
        )
        .option("checkpointLocation", settings.checkpoint_invalid_cdc_users)
        .trigger(processingTime=settings.trigger_interval)
        .start()
    )

    _ = (query_raw, query_invalid)
