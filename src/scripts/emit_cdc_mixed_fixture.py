"""Emit deterministic mixed CDC fixtures for CDC-CONTRACT contract validation."""
...
def build_mixed_fixture_records(
    ...
    *,
    ...
    records.append(FixtureRecord(video_id, "not-json-{cdc-contract", False))
    records.append(
        FixtureRecord(
            video_id,
            json.dumps(
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Emit deterministic mixed CDC fixture for CDC-CONTRACT")
...
def _emit_to_kafka(topic: str, bootstrap_servers: str, records: List[FixtureRecord], sleep_ms: int) -> None:
    ...
    producer = Producer({"bootstrap.servers": bootstrap_servers, "client.id": "cdc-contract-mixed-emitter"})
