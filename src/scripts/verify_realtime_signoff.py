"""Unified RT-SIGNOFF Sprint 1 sign-off verifier."""
...
def build_signoff_report(
    *,
    ...
    gate = _gate(
        ...
        "All CDC-UPSERT/39/40/43 verifier commands reported PASS."
    ...
    report = {
        ...
        "blockers": failures,
        "carry_over_items": [
            f"Resolve gate `{gate_row['name']}` and re-run RT-SIGNOFF acceptance."
            for gate_row in gates
            if not gate_row["passed"]
        ],
    }
...
def render_markdown_summary(report: Mapping[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# RT-SIGNOFF Sprint 1 Sign-off Summary")
    lines.append("")
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Unified RT-SIGNOFF Sprint 1 sign-off verifier")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if report["status"] == "PASS":
        print("PASS: RT-SIGNOFF unified sign-off verifier passed")
        print(json.dumps({"status": report["status"], "report_json": str(report_json_path)}, sort_keys=True))
        return 0

    print("FAIL: RT-SIGNOFF unified sign-off verifier failed")
