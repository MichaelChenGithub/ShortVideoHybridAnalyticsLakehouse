...
class RtVideoCdcJobSettingsTests(unittest.TestCase):
    def test_load_job_settings_defaults(self) -> None:
        ...
        self.assertEqual(settings.checkpoint_raw_cdc, CHECKPOINT_RAW_CDC_VIDEOS)
        self.assertEqual(settings.checkpoint_invalid_cdc, CHECKPOINT_INVALID_CDC_VIDEOS)
        ...
        self.assertEqual(settings.raw_cdc_table, RAW_CDC_TABLE)
        self.assertEqual(settings.invalid_cdc_table, INVALID_CDC_TABLE)
    ...
    def test_load_job_settings_honors_env_overrides(self) -> None:
        settings = load_job_settings(
            {
                ...
                ENV_CHECKPOINT_RAW_CDC_VIDEOS: "s3a://checkpoints/custom/raw/v1",
                ENV_CHECKPOINT_INVALID_CDC_VIDEOS: "s3a://checkpoints/custom/invalid/v1",
                ENV_CONSUMER_GROUP: "cg_custom",
                ENV_DIM_VIDEOS_TABLE: "lakehouse.dims.custom_dim_videos",
                ENV_RAW_CDC_TABLE: "lakehouse.bronze.custom_raw_cdc",
                ENV_INVALID_CDC_TABLE: "lakehouse.bronze.custom_invalid_cdc",
            }
        )
        ...
        self.assertEqual(settings.checkpoint_raw_cdc, "s3a://checkpoints/custom/raw/v1")
        self.assertEqual(settings.checkpoint_invalid_cdc, "s3a://checkpoints/custom/invalid/v1")
        ...
        self.assertEqual(settings.raw_cdc_table, "lakehouse.bronze.custom_raw_cdc")
        self.assertEqual(settings.invalid_cdc_table, "lakehouse.bronze.custom_invalid_cdc")
