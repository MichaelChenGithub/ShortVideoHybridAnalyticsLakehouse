...
from scripts.verify_realtime_signoff import (  # noqa: E402
...
class VerifyRealtimeSignoffTests(unittest.TestCase):
    ...
    def test_build_signoff_report_passes_for_healthy_inputs(self) -> None:
        ...
        report = build_signoff_report(
            run_id="realtime_signoff_healthy",
            ...
            cdc_upsert={
                "video_id": "realtime_signoff_healthy_cdc_vid_001",
    ...
    def test_build_signoff_report_fails_for_multiple_gate_breaches(self) -> None:
        now_ms = 2_000_000
        report = build_signoff_report(
            run_id="realtime_signoff_unhealthy",
            run_start_ms=1_900_000,
            verifier_status={
                "content_metrics": "PASS",
                "content_contract": "PASS",
            ...
            cdc_upsert={
                "video_id": "realtime_signoff_unhealthy_cdc_vid_001",
    ...
    def test_build_signoff_report_uses_runtime_end_sample_at_for_freshness(self) -> None:
        ...
        report = build_signoff_report(
            run_id="realtime_signoff_checked_at",
            ...
            cdc_upsert={
                "video_id": "realtime_signoff_checked_at_cdc_vid_001",
        ...
        self.assertEqual(report["status"], "FAIL")
        self.assertEqual(report["checked_at_ms"], final_now_ms)

        freshness_gate = next(gate for gate in report["gates"] if gate["name"] == "freshness_sla")
        self.assertFalse(freshness_gate["passed"])
        self.assertEqual(freshness_gate["metrics"]["content_freshness_age_ms"], 420_000)
        self.assertEqual(freshness_gate["metrics"]["content_invalid_freshness_age_ms"], 410_000)
        self.assertEqual(freshness_gate["metrics"]["cdc_freshness_age_ms"], 100_000)
