...
from spark.bt_dim_videos_scd2 import (  # noqa: E402
    ENV_APP_NAME,
    ENV_SOURCE_TABLE,
    ENV_TARGET_TABLE,
    JobSettings,
...
class BtDimVideosScd2TransformTests(unittest.TestCase):
    def test_load_job_settings_defaults(self) -> None:
        settings = load_job_settings({})
        self.assertEqual(settings.app_name, "spark_bt_dim_videos_scd2")
        self.assertEqual(settings.source_table, RAW_CDC_TABLE)
        self.assertEqual(settings.target_table, DIM_VIDEOS_SCD2_TABLE)

    def test_load_job_settings_overrides(self) -> None:
        settings = load_job_settings(
            {
                ENV_APP_NAME: "custom_bt_dim_videos_scd2",
                ENV_SOURCE_TABLE: "lakehouse.bronze.raw_cdc_videos_canary",
                ENV_TARGET_TABLE: "lakehouse.dims.dim_videos_scd2_canary",
            }
        )
        self.assertEqual(settings.app_name, "custom_bt_dim_videos_scd2")
        self.assertEqual(settings.source_table, "lakehouse.bronze.raw_cdc_videos_canary")
        self.assertEqual(settings.target_table, "lakehouse.dims.dim_videos_scd2_canary")

    @patch("spark.bt_dim_videos_scd2.run_batch")
    @patch("spark.bt_dim_videos_scd2.load_job_settings")
    ...
    def test_run_batch_issues_create_and_overwrite_sql(self) -> None:
        settings = JobSettings(
            app_name="spark_bt_dim_videos_scd2_test",
            source_table="lakehouse.bronze.raw_cdc_videos_canary",
            target_table="lakehouse.dims.dim_videos_scd2_canary",
        )
        fake_spark = _FakeSpark(counts=[12, 7])
        ...
        self.assertIn(
            "CREATE TABLE IF NOT EXISTS lakehouse.dims.dim_videos_scd2_canary",
