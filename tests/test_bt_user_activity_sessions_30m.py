...
from spark.bt_user_activity_sessions_30m import (  # noqa: E402
    ENV_APP_NAME,
    ENV_DATA_DATE,
    ENV_DIM_USERS_TABLE,
    ENV_SOURCE_TABLE,
...
class BtUserActivitySessions30mTransformTests(unittest.TestCase):
    def test_load_job_settings_defaults(self) -> None:
        settings = load_job_settings({})
        self.assertEqual(settings.app_name, "spark_bt_user_activity_sessions_30m")
        self.assertEqual(settings.source_table, EVENTS_CONFORMED_TABLE)
        self.assertEqual(settings.dim_users_table, DIM_USERS_SCD2_TABLE)
        self.assertEqual(settings.target_table, USER_ACTIVITY_SESSIONS_30M_TABLE)
        self.assertIsNone(settings.data_date_sql)

    def test_load_job_settings_overrides(self) -> None:
        settings = load_job_settings(
            {
                ENV_APP_NAME: "custom_bt_user_activity_sessions_30m",
                ENV_SOURCE_TABLE: "lakehouse.silver.events_conformed_canary",
                ENV_DIM_USERS_TABLE: "lakehouse.dims.dim_users_scd2_canary",
                ENV_TARGET_TABLE: "lakehouse.silver.user_activity_sessions_30m_canary",
                ENV_DATA_DATE: "2026-03-20",
            }
        )
        self.assertEqual(settings.app_name, "custom_bt_user_activity_sessions_30m")
        self.assertEqual(settings.source_table, "lakehouse.silver.events_conformed_canary")
        self.assertEqual(settings.dim_users_table, "lakehouse.dims.dim_users_scd2_canary")
        self.assertEqual(settings.target_table, "lakehouse.silver.user_activity_sessions_30m_canary")
        self.assertEqual(settings.data_date_sql, "DATE '2026-03-20'")

    @patch("spark.bt_user_activity_sessions_30m.run_batch")
    @patch("spark.bt_user_activity_sessions_30m.load_job_settings")
    ...
    def test_run_batch_issues_create_delete_and_insert_sql(self) -> None:
        settings = JobSettings(
            app_name="spark_bt_user_activity_sessions_30m_test",
        ...
        self.assertEqual(
            ...
            create_user_activity_sessions_30m_sql(settings.target_table),
        )
        self.assertEqual(
