# network.tf — VPC, subnets, IGW, NAT gateway, route tables, security groups
...
resource "aws_subnet" "public_a" {
  ...
  tags = { Name = "${var.project_name}-public-a", Project = var.project_name }
}
...
resource "aws_route_table" "private" {
  ...
  tags = { Name = "${var.project_name}-rt-private", Project = var.project_name }
}
...
# Shared SG for ECS Fargate tasks and EMR Serverless VPC workers.
# Ingress is handled per-service (e.g. messaging.tf adds MSK ingress from this SG).
resource "aws_security_group" "ecs_tasks" {
  name        = "${var.project_name}-ecs-tasks-sg"
  description = "ECS Fargate tasks and EMR Serverless: unrestricted egress; ingress per service"
  vpc_id      = aws_vpc.main.id
